...
const movieGenreModel = {
  ...
  async insertIntoMovieGenre(movie, genre) {
    try {
      // Check if the genre/film pair is already present in the database
      const userQuery = {
          text: 'SELECT * FROM "movie_genre" WHERE movie_id = $1 AND genre_id = $2',
          values: [movie.id, genre.id],
      };
      ...
      const insertQuery = {
          text: 'INSERT INTO "movie_genre" (movie_id, genre_id) VALUES ($1, $2) RETURNING *',
          values: [movie.id, genre.id],
      };
