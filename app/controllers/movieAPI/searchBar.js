...
async function fetchBySearchBar(req, res) {
  ...
  try {
    ...
    const responseByTitle = await fetch(
      `${API_TMDB_BASE_URL}search/movie?api_key=${API_TMDB_KEY}&query=${encodedSearchTerm}&language=fr-FR`
    );

    // Search by genre
    const genreResponse = await fetch(
      `${API_TMDB_BASE_URL}/genre/movie/list?api_key=${API_TMDB_KEY}&language=fr-FR`
    );
    if (!genreResponse.ok) {
      throw new Error(
        "Erreur réseau ou réponse non valide lors de la récupération des genres"
      );
    }
    ...
    const responseByActor = await fetch(
      `${API_TMDB_BASE_URL}search/person?api_key=${API_TMDB_KEY}&query=${encodedSearchTerm}&language=fr-FR`
    );
    ...
    let moviesByGenre = [];
    if (genreId) {
      const responseByGenre = await fetch(
        `${API_TMDB_BASE_URL}/discover/movie?api_key=${API_TMDB_KEY}&language=fr-FR&sort_by=popularity.desc&with_genres=${genreId}`
      );
    ...
    const moviesByTitle = moviesByTitleData.results.map((movie) => ({
      id: movie.id,
      title: movie.title,
    ...
    const moviesByActorPromises = moviesByActorData.results.map(
      async (actor) => {
        const response = await fetch(
          `${API_TMDB_BASE_URL}person/${actor.id}/movie_credits?api_key=${API_TMDB_KEY}&language=fr-FR`
        );
        if (response.ok) {
          const credits = await response.json();
          return credits.cast.map((movie) => ({
            id: movie.id,
            title: movie.title,
            poster_path: movie.poster_path,
          }));
    ...
    debug(
      "Erreur lors de la récupération des films par titre, genre ou acteur :",
      error
    );
