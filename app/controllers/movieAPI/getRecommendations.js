...
async function getRecommendations(movieId, language, page) {
  ...
  try {
    ...
    const response = await fetch(
      `${API_TMDB_BASE_URL}movie/${movieId}/recommendations?api_key=${API_TMDB_KEY}&language=${language}&page=${page}`
    );
    ...
    const recommendationsWithProvidersPromises = recommendationResults.map(
      async (movie) => {
    ...
    const recommendationsWithProviders = await Promise.all(
      recommendationsWithProvidersPromises
    );
